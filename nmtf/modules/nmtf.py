...
class NMF:
    def __init__(
        ...
        -------
        >>> from nmtf import *
        >>> my_mm_fmodel = NMF(n_components=4)

        References
        ----------

            P. Fogel, D.m. Hawkins, x. Beecher, G. Luta, S. S. Young (2013). a Tale of Two Matrix Factorizations.
    ...
    def fit_transform(
        ...
        convex hull will be given largest weights. The column weight is a function of the skewness and its sign. The
        expected sign of the skewness is based on the skewness of w components, as returned by the first pass of a
        2-steps convex NMF. Thus, during the first pass, skewness must be set to False. Can be set only if convex =
        'transformation' and prior w and h have been defined.
        ...
        WB : array-like, shape (n_samples, n_components) a sample is clustered in cluster k if its the_leverage on
        component k is higher than on any other components. During each run of the bootstrap, samples are
        re-clustered. Each row of WB contains the frequencies of the n_components clusters following the bootstrap.
        Only if n_bootstrap > 0.

        HB : array-like, shape (n_components, n_features) a feature is clustered in cluster k if its the_leverage on
        component k is higher than on any other components. During each run of the bootstrap, features are
        re-clustered. Each row of HB contains the frequencies of the n_components clusters following the bootstrap.
        Only if n_bootstrap > 0.
        ...
        >>> estimator = my_mm_fmodel.fit_transform(m)
    ...
    def predict(self, estimator, blocks=None, cluster_by_stability=False, custom_order=False):
        ...
        >>> my_mm_fmodel = NMF(n_components=4)
        >>> # m: matrix to be factorized
        >>> estimator1 = my_mm_fmodel.fit_transform(m)
        >>> estimator2 = my_mm_fmodel.predict(estimator)
    ...
    def permutation_test_score(self, estimator, y, n_permutations=100):
        ...
        >>> # m: matrix to be factorized
        >>> estimator1 = my_mm_fmodel.fit_transform(m)
        >>> # row_groups: the group each sample is associated with
        >>> estimator2 = my_mm_fmodel.permutation_test_score(estimator, row_groups, n_permutations=100)
...
class NTF:
    ...
    -------
    >>> from nmtf import *
    >>> my_nt_fmodel = NTF(n_components=4)
    ...
    def fit_transform(
        ...
        >>> estimator = my_nt_fmodel.fit_transform(m, 5)

        Reference
        ---------

        a. Cichocki, P.h.a.N. Anh-Huym, Fast local algorithms for large scale nonnegative matrix and tensor
        factorizations, IEICE Trans. Fundam. Electron. Commun. Comput. Sci. 92 (3) (2009) 708–721.
